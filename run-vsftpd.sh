...
if [ ! "$FTP_USERS" = "**String**" ]; then
	echo -e "$FTP_USERS" > /etc/vsftpd/virtual_users.txt
fi
...
cat /etc/vsftpd/vsftpd.conf.backup > /etc/vsftpd/vsftpd.conf
...
echo "cmds_denied=${CMDS_DENIED}" >> /etc/vsftpd/vsftpd.conf
